...
![Resumen Lighthouse de Reportes](../assets/img/chapter-VIII/lighthouse-reportes-overview.png)
 
![Detalle de Performance de Reportes](../assets/img/chapter-VIII/lighthouse-reportes-performance.png)
 
**Performance (80):** Puntaje "naranja" (mejorable). El First Contentful Paint se ubica en 1.2s, el Largest Contentful Paint en 1.7s, el Speed Index en 1.9s y el Total Blocking Time en 210 ms. Aunque muestra un rendimiento significativamente más rápido que otras secciones y una latencia base inicial aceptable, el LCP y el Speed Index superan ligeramente el umbral de tolerancia de 1.5s definido para H4 (8.2.4).
 
![Detalle de Accessibility de Reportes](../assets/img/chapter-VIII/lighthouse-reportes-accessibility.png)
 
**Accessibility (90):** Puntaje alto (verde). No obstante, se detectan fallos menores en elementos interactivos como la falta de nombres accesibles en botones ("Buttons do not have an accessible name"). Además, se mantiene de manera transversal la alerta de contraste insuficiente entre el fondo y el primer plano, confirmando el problema de diseño descrito en el As-Is (8.1.1) y justificando la validez del experimento de Hipótesis 5.
 
![Detalle de Best Practices de Reportes](../assets/img/chapter-VIII/lighthouse-reportes-best-practices.png)
 
**Best Practices (96):** Excelente cumplimiento de estándares. Se registran advertencias no críticas relacionadas con errores en consola ("Browser errors were logged to the console") y la ausencia de source maps en JavaScript de primera parte ("Missing source maps for large first-party JavaScript").
 
![Detalle de SEO de Reportes](../assets/img/chapter-VIII/lighthouse-reportes-seo.png)
 
**SEO (82):** Puntaje aceptable. Se reportan alertas de meta descripción ausente y un archivo `robots.txt` no válido con 15 errores. Al tratarse de una vista interna para usuarios autenticados, este aspecto es de baja prioridad para el éxito del producto.
...
**SEO (75):** Mismos hallazgos que en la pantalla de Reportes (meta descripción ausente, `robots.txt` inválido); poco relevante dado que es una pantalla protegida por login.
